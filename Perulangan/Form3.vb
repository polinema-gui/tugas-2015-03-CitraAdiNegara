Public Class Form3
    ...
    Private Sub btn_proses_Click(sender As Object, e As EventArgs) Handles btn_proses.Click
        Try
            Dim barisMatriks1 As Integer = Tbl_matrik1.Lines.Count
            Dim kolomMatriks1 As Integer = Tbl_matrik1.Lines.ElementAt(0).Split(" ").Length
            Dim barisMatriks2 As Integer = Tbl_matrik1.Lines.Count
            Dim kolomMatriks2 As Integer = Tbl_matrik1.Lines.ElementAt(0).Split(" ").Length
        ...
        If (Me.cekKolom) Then
            For baris As Integer = 0 To baris1 - 1
                For kolom As Integer = 0 To kolom1 - 1
                    nilai1(baris, kolom) = Tbl_matrik1.Lines.ElementAt(baris).Split(" ").ElementAt(kolom)
                    nilai2(baris, kolom) = Tbl_matrik2.Lines.ElementAt(baris).Split(" ").ElementAt(kolom)
                Next
            Next
        End If
    End Sub
    ...
    Private Sub cek(ByVal baris1 As Integer, baris2 As Integer, kolom1 As Integer, kolom2 As Integer)
        ...
        For baris = 0 To baris1 - 2
            If (Tbl_matrik1.Lines.ElementAt(baris).Split(" ").Length <> Tbl_matrik1.Lines.ElementAt(baris + 1).Split(" ").Length) Then
                tmp = False
            End If
        Next
        For baris = 0 To baris2 - 2
            If (Tbl_matrik2.Lines.ElementAt(baris).Split(" ").Length <> Tbl_matrik2.Lines.ElementAt(baris + 1).Split(" ").Length) Then
                tmp = False
            End If
        Next
        If (tmp = False) Then
            Me.cekKolom = False
            Convert.ToInt32(Tbl_matrik1.Text)
            Convert.ToInt32(Tbl_matrik2.Text)
